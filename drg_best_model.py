...
STATIC_N_STRIPS = 7
STATIC_ORIGINAL_IMAGE_INFLUENCE = 0.4
STATIC_PHASE_NOISE_STD = 0.03
STATIC_SMOOTH_SIGMA = 0.3
...
AUGMENTATION_STRENGTH_TRAIN = "strong"
LOSS_TYPE = "mae_gdl"
LAMBDA_GDL = 0.3
LEARNING_RATE = 1e-3
BATCH_SIZE = 8
...
def generate_cubic_background(shape: tuple, n_strips: int, tilt_angle_deg: float, curvature_amplitude: float, background_offset: float) -> np.ndarray:
    """
    shape: Rozmery obrázka (H, W).
    n_strips: Počet prúžkov.
    tilt_angle_deg: Uhol náklonu v stupňoch.
    curvature_amplitude: Amplitúda zakrivenia.
    background_offset: Posunutie pozadia.
    """
    H, W = shape
    ...
    a_val = 0.0 * curvature_amplitude
    b_val = -4.0 * curvature_amplitude
    c_val = 4.0 * curvature_amplitude
    d_val = background_offset
    poly1d = a_val * x_norm**3 + b_val * x_norm**2 + c_val * x_norm + d_val
    poly2d = np.tile(poly1d, (H, 1))
    background = linear_grad + poly2d
    return background

def generate_simulation_pair(source_img_np: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """
    source_img_np: Zdrojový obrázok ako numpy pole.
    """
    if STATIC_SMOOTH_SIGMA > 0:
        base_img = gaussian(source_img_np, sigma=STATIC_SMOOTH_SIGMA, preserve_range=True)
    else:
    ...
    background = generate_cubic_background(source_img_np.shape,
                                           ...
                                           1.7,
                                           -15.0)
    unwrapped = object_phase * STATIC_ORIGINAL_IMAGE_INFLUENCE + background * (1.0 - STATIC_ORIGINAL_IMAGE_INFLUENCE)
    if STATIC_PHASE_NOISE_STD > 0:
        unwrapped += np.random.normal(0, STATIC_PHASE_NOISE_STD, size=source_img_np.shape)
...
class DynamicTrainPhaseDataset(torch.utils.data.Dataset):
    def __init__(self, source_image_filepaths: list[str], norm_stats_input_global: tuple[float, float], norm_stats_target_global: tuple[float, float], augmentation_strength: str = 'none', target_img_size: tuple[int, int] = (512,512)):
        """
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        unwrapped_phase_np, wrapped_phase_np = generate_simulation_pair(source_img_norm)
...
def train_evaluate_model(global_input_stats: tuple[float, float], global_target_stats: tuple[float, float]):
    """
    ...
    train_ds = DynamicTrainPhaseDataset(train_source_files, global_input_stats, global_target_stats, AUGMENTATION_STRENGTH_TRAIN, TARGET_IMG_SIZE)
    ...
    valid_loader = DataLoader(valid_ds, batch_size=BATCH_SIZE, shuffle=False, num_workers=1)
...
if __name__ == '__main__':
    ...
    train_evaluate_model(input_stats, target_stats)