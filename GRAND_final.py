...
if __name__ == '__main__':
    CONFIG_PATH_DRG = r"C:\Users\juraj\Desktop\TRENOVANIE_bakalarka_simul\optimalizacia_hype\trenovanie_5\config_R34imgnet_direct_MAE_GDL0p3_AugMedium_LR5em04_WD1em04_Ep120_ESp30_Tmax120_EtaMin1em07_bs8_bs8.txt" 
    WEIGHTS_PATH_DRG = r"C:\Users\juraj\Desktop\TRENOVANIE_bakalarka_simul\optimalizacia_hype\trenovanie_5\best_weights_R34imgnet_direct_MAE_GDL0p3_AugMedium_LR5em04_WD1em04_Ep120_ESp30_Tmax120_EtaMin1em07_bs8_bs8.pth"
    CONFIG_PATH_DWC = r"C:\Users\juraj\Desktop\TRENOVANIE_bakalarka_simul\classification\experiment4_hyper\config_clf_R34imgnet_Kmax6_AugMed_LR1em03_WD1em04_Ep120_Tmax120_EtaMin1em07_EdgeW5.0_bs8.txt"
    WEIGHTS_PATH_DWC = r"C:\Users\juraj\Desktop\TRENOVANIE_bakalarka_simul\classification\experiment4_hyper\best_weights_clf_R34imgnet_Kmax6_AugMed_LR1em03_WD1em04_Ep120_Tmax120_EtaMin1em07_EdgeW5.0_bs8.pth"
    TEST_DATA_PATH_ROOT = r'C:\Users\juraj\Desktop\TRENOVANIE_bakalarka_simul\split_dataset_tiff_for_dynamic_v_stratified_final\static_test_dataset'
    GOLDSTEIN_RESULTS_PATH = r"C:\\Users\\juraj\\Desktop\\matlab_goldstain\\2d-phase-unwrap-goldstein\\goldstein_unwrapped"
    OUTPUT_COMPARISON_DIR = r"C:\Users\juraj\Desktop\POROVNANIE_METOD_FINALNE_V3"
    DEVICE_TO_USE = 'cuda'
    ...
    with open(stats_output_path, 'w', encoding='utf-8') as f:
        ...
        for method_key in method_names_ordered:
            ...
            if len(maes) > 0:
                ...
                f.write(f"    Min:       {np.min(maes):.6f} rad\n")
                f.write(f"    Max:       {np.max(maes):.6f} rad\n")
            else: f.write(f"  MAE: Žiadne platné dáta.\n")
            
            if len(psnrs) > 0:
                f.write(f"  PSNR (na obrázok):\n")
                f.write(f"    Priemer:   {np.mean(psnrs):.2f} dB\n")
                f.write(f"    Medián:    {np.median(psnrs):.2f} dB\n")
                f.write(f"    Std.odch:  {np.std(psnrs):.2f} dB\n")
            else: f.write(f"  PSNR: Žiadne platné dáta.\n")

            if len(ssims) > 0:
                f.write(f"  SSIM (na obrázok):\n")
                f.write(f"    Priemer:   {np.mean(ssims):.4f}\n")
                f.write(f"    Medián:    {np.median(ssims):.4f}\n")
                f.write(f"    Std.odch:  {np.std(ssims):.4f}\n")
    ...
    if plot_data_boxplot:
        ...
        plt.ylabel("Stredná absolútna chyba (MAE) na obrázok [rad]", fontsize=12)
