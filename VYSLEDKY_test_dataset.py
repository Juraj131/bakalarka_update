...
def evaluate_and_visualize_model(
    ...
    """
    Načíta model, evaluuje ho na testovacom datasete a zobrazí vybraný obrázok.
    """
    ...
    # Extrakcia potrebných parametrov z konfigurácie
    # (s defaultnými hodnotami, ak by kľúč chýbal - prispôsob podľa potreby)
    encoder_name = config.get("Encoder Name", "resnet18")
    input_processing_type = config.get("Input Processing", "sincos") # 'sincos' alebo 'direct_minmax'
    
    # Štatistiky pre normalizáciu
    # Pre unwrapped (cieľ) - Z-score
    ...
    if target_original_std < 1e-6:
        print("VAROVANIE: Načítaná Std pre cieľ je veľmi nízka, môže spôsobiť problémy.")
        # target_original_std = 1.0 # Núdzové riešenie

    # Pre wrapped (vstup) - MinMax, ak sa používa direct_minmax
    ...
    if input_processing_type == 'direct_minmax':
        ...
        if input_min == input_max:
            print("VAROVANIE: Načítané Min a Max pre vstup sú rovnaké. Normalizácia môže byť problematická.")
            # input_max = input_min + 1e-6 # Núdzové riešenie
    ...
    test_dataset = CustomDataset(
        ...
        augmentation_strength='none', # Pre testovanie žiadne augmentácie
        is_train_set=False
    )
    ...
    # Overenie image_index_to_show
    if not (0 <= image_index_to_show < len(test_dataset)):
        print(f"CHYBA: Zadaný index obrázka {image_index_to_show} je mimo rozsahu testovacieho datasetu (0-{len(test_dataset)-1}).")
        print(f"Pre vizualizáciu bude použitý prvý obrázok (index 0).")
        image_index_to_show = 0

    # DataLoader pre celý testovací set (pre celkové metriky)
    ...
    # Predpokladáme, že sa vždy používa smp.Unet, keďže CustomResUNet sa nepoužíva
    # a config nemusí obsahovať 'Model Architecture'
    # Ak by config obsahoval 'Model Architecture' a nebolo by to 'smp_unet', vypíšeme varovanie.
    ...
    try:
        net = smp.Unet(
            ...
            encoder_weights=None, # Váhy sa načítajú zo súboru, nepotrebujeme 'imagenet'
        ...
        print("Skontroluj, či je názov enkodéra správny a podporovaný knižnicou segmentation-models-pytorch.")
        return
    ...
    all_psnr = []
    all_ssim = []
    all_gdl = [] # Nové pre GDL
    
    worst_mae_sample = {"mae": -1.0, "index": -1}
    best_mae_sample  = {"mae": float('inf'), "index": -1}
    
    most_structured_gt_sample = {"variance": -1.0, "index": -1}
    
    best_gdl_sample = {"gdl": float('inf'), "index": -1} 
    worst_gdl_sample = {"gdl": -1.0, "index": -1}    

    all_sample_eval_metrics = [] # Pre výpočet kompromisného indexu
    ...
    with torch.no_grad():
        for i, batch_data in enumerate(test_loader):
            ...
            for k in range(pred_norm_batch.size(0)):
                ...
                pred_norm_sample = pred_norm_batch[k]
                target_orig_sample_numpy = target_orig_batch[k].cpu().numpy().squeeze() 

                pred_denorm_sample = denormalize_target_z_score(pred_norm_sample, target_original_mean, target_original_std)
                pred_denorm_sample_numpy = pred_denorm_sample.cpu().numpy().squeeze()
                ...
                gdl_val = calculate_gdl_metric(pred_denorm_sample_numpy, target_orig_sample_numpy)
                
                all_mae_denorm.append(mae)
                all_mse_denorm.append(mse)
                if not np.isnan(gdl_val): all_gdl.append(gdl_val)
                ...
                if mae < best_mae_sample["mae"]:
                    best_mae_sample["mae"] = mae
                    best_mae_sample["index"] = current_sample_idx
                if mae > worst_mae_sample["mae"]: 
                    worst_mae_sample["mae"] = mae
                    worst_mae_sample["index"] = current_sample_idx
                
                gt_variance = np.var(target_orig_sample_numpy)
                if gt_variance > most_structured_gt_sample["variance"]:
                    most_structured_gt_sample["variance"] = gt_variance
                    most_structured_gt_sample["index"] = current_sample_idx

                if not np.isnan(gdl_val):
                    if gdl_val < best_gdl_sample["gdl"]:
                        best_gdl_sample["gdl"] = gdl_val
                        best_gdl_sample["index"] = current_sample_idx
                    if gdl_val > worst_gdl_sample["gdl"]: # Inicializované na -1, takže prvá platná hodnota bude väčšia
                        worst_gdl_sample["gdl"] = gdl_val
                        worst_gdl_sample["index"] = current_sample_idx
                
                all_sample_eval_metrics.append({
                    "index": current_sample_idx,
                    "mae": mae,
    ...
    avg_gdl = np.mean(all_gdl) if all_gdl else np.nan
    avg_psnr = np.mean(all_psnr) if all_psnr else np.nan # Už predtým bolo np.mean, čo je OK pre zoznam bez NaN
    ...
    print(f"Priemerná GDL (denormalizovaná): {avg_gdl:.4f}")
    if SKIMAGE_AVAILABLE:
        print(f"Priemerný PSNR: {avg_psnr:.2f} dB")
        print(f"Priemerný SSIM: {avg_ssim:.4f}")
    
    print("\n--- Extrémne Hodnoty a Zaujímavé Indexy (len logovanie) ---")
    print(f"Najlepšia MAE na vzorke: {best_mae_sample['mae']:.4f} (index: {best_mae_sample['index']})")
    print(f"Najhoršia MAE na vzorke: {worst_mae_sample['mae']:.4f} (index: {worst_mae_sample['index']})")
    print(f"Obrázok s najvyššou varianciou GT: {most_structured_gt_sample['variance']:.4f} (index: {most_structured_gt_sample['index']})")
    if best_gdl_sample['index'] != -1:
        print(f"Najlepšia GDL na vzorke (najnižšia): {best_gdl_sample['gdl']:.4f} (index: {best_gdl_sample['index']})")
    if worst_gdl_sample['index'] != -1:
        print(f"Najhoršia GDL na vzorke (najvyššia): {worst_gdl_sample['gdl']:.4f} (index: {worst_gdl_sample['index']})")

    compromise_index = -1
    if all_sample_eval_metrics:
        # Normalizácia MAE (0=najlepšia, 1=najhoršia)
        maes_list = np.array([s['mae'] for s in all_sample_eval_metrics])
        min_mae_val, max_mae_val = np.min(maes_list), np.max(maes_list)
        norm_maes = (maes_list - min_mae_val) / (max_mae_val - min_mae_val + 1e-9)
        
        # Normalizácia GT variancie (1=najlepšia, 0=najhoršia)
        gt_vars_list = np.array([s['gt_variance'] for s in all_sample_eval_metrics])
        min_gt_var_val, max_gt_var_val = np.min(gt_vars_list), np.max(gt_vars_list)
        norm_gt_vars = (gt_vars_list - min_gt_var_val) / (max_gt_var_val - min_gt_var_val + 1e-9)
        
        compromise_scores = (1.0 - norm_maes) + norm_gt_vars # Vyššie je lepšie
        
        if len(compromise_scores) > 0:
            best_compromise_score_idx_local = np.argmax(compromise_scores)
            compromise_index = all_sample_eval_metrics[best_compromise_score_idx_local]['index']
            best_compromise_score_val = compromise_scores[best_compromise_score_idx_local]
            print(f"Obrázok s najlepším kompromisom (nízka MAE, vysoká GT var.): index {compromise_index} (skóre: {best_compromise_score_val:.4f})")
        else:
            print("Nepodarilo sa vypočítať kompromisný index (žiadne dáta).")


    # --- Vizualizácia Vybraného Obrázka (podľa image_index_to_show) ---
    print(f"\nVizualizujem a ukladám obrázok s indexom: {image_index_to_show}")
    
    try:
        vis_input_norm, _, vis_target_orig = test_dataset[image_index_to_show] 
        if vis_input_norm is None:
            print(f"CHYBA: Nepodarilo sa načítať obrázok s indexom {image_index_to_show} pre vizualizáciu.")
            return # Návrat z funkcie, ak sa obrázok nepodarí načítať
    except IndexError:
        print(f"CHYBA: Index {image_index_to_show} je mimo rozsahu datasetu ({len(test_dataset)}).")
        return # Návrat z funkcie

    vis_input_norm_dev = vis_input_norm.unsqueeze(0).to(device) 

    with torch.no_grad():
        vis_pred_norm = net(vis_input_norm_dev).squeeze(0) 

    vis_pred_denorm_numpy = denormalize_target_z_score(vis_pred_norm.cpu(), target_original_mean, target_original_std).numpy().squeeze()
    vis_gt_denorm_numpy = vis_target_orig.numpy().squeeze() 

    input_display_title = "Input (Spracovaný)"
    input_display_data = None
    vmin_in, vmax_in = None, None

    if input_processing_type == 'sincos':
        input_display_data = vis_input_norm[0].cpu().numpy().squeeze() 
        vmin_in, vmax_in = -1, 1
        input_display_title = "Input (sin(φ) kanál)"
    elif input_processing_type == 'direct_minmax':
        input_display_data = vis_input_norm.cpu().numpy().squeeze()
        vmin_in, vmax_in = -1, 1
        input_display_title = "Input (Wrapped, norm. to [-1,1])"
    
    plt.figure(figsize=(18, 6)) # Vytvor nový obrázok pre túto vizualizáciu
    run_name_for_title = os.path.splitext(os.path.basename(weights_path))[0].replace("best_weights_", "")
    plt.suptitle(f"Test Visualization - Obrázok index {image_index_to_show}\nRun: {run_name_for_title}", fontsize=14)
    
    plt.subplot(1, 3, 1)
    if input_display_data is not None:
        im1 = plt.imshow(input_display_data, cmap='gray', vmin=vmin_in, vmax=vmax_in)
        plt.title(input_display_title)
        plt.colorbar(im1)
    else:
        plt.text(0.5, 0.5, "Vstup nie je zobrazený", ha='center', va='center'); plt.axis('off')

    plt.subplot(1, 3, 2)
    im2 = plt.imshow(vis_gt_denorm_numpy, cmap='gray')
    plt.title("Ground Truth (denormalizovaný)")
    plt.colorbar(im2)
    
    plt.subplot(1, 3, 3)
    common_min = min(vis_gt_denorm_numpy.min(), vis_pred_denorm_numpy.min())
    common_max = max(vis_gt_denorm_numpy.max(), vis_pred_denorm_numpy.max())
    im3 = plt.imshow(vis_pred_denorm_numpy, cmap='gray', vmin=common_min, vmax=common_max)
    plt.title("Predikcia (denormalizovaná)")
    plt.colorbar(im3)
    
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    save_fig_path = f"visualization_idx{image_index_to_show}_{run_name_for_title}.png"
    plt.savefig(save_fig_path)
    print(f"Vizualizácia uložená do: {save_fig_path}")
    plt.show() # Zobrazí len tento posledný vygenerovaný obrázok


if __name__ == '__main__':
    # --- NASTAVENIA PRE TESTOVANIE ---
    # Zmeň tieto cesty a parametre podľa potreby
    
    # Cesta ku konfiguračnému súboru experimentu, ktorého výsledky chceš analyzovať
    # Tento súbor by mal byť vygenerovaný počas tréningu
    CONFIG_FILE_PATH = r"C:\Users\viera\Desktop\q_tiff\TRENOVANIE_BAKALARKA_simul\regres\experiment2\trenovanie7\config_R34_direct_MAE_GDL0.1_AugStrong_bs8_Lgdl0p3.txt" # PRÍKLAD! UPRAV!
    
    # Cesta k uloženým váham modelu z daného experimentu
    WEIGHTS_FILE_PATH = r"C:\Users\viera\Desktop\q_tiff\TRENOVANIE_BAKALARKA_simul\regres\experiment2\trenovanie7\best_weights_R34_direct_MAE_GDL0.1_AugStrong_bs8_Lgdl0p3.pth" # PRÍKLAD! UPRAV!

    # Cesta k adresáru s testovacím datasetom
    TEST_DATA_PATH = r'C:\Users\viera\Desktop\q_tiff\TRENOVANIE_bakalarka_simul\split_dataset_tiff\test_dataset' # UPRAV PODĽA POTREBY

    # Index obrázka z testovacieho datasetu, ktorý sa má zobraziť (začína od 0)
    IMAGE_INDEX_TO_VISUALIZE = 108

    DEVICE_TO_USE = 'cuda' # 'cuda' alebo 'cpu'

    # --- Spustenie evaluácie a vizualizácie ---
    if not os.path.exists(CONFIG_FILE_PATH):
        ...
        evaluate_and_visualize_model(
            ...
            image_index_to_show=IMAGE_INDEX_TO_VISUALIZE,
